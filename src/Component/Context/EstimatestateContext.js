...
const EstimatestateContext = ({ children }) => {
  ...
  let columnNames = [
    { columnname: "S.no", display: true, cnmae: "other" },
  ...
  const saveEstimationDetailCounter = async (estimateidcount, loginuserid) => {
    ...
    if (isbackendconnect) {
      responsesaveEstimationId = await estimateDetailsDb.saveEstimationId(
        estimateidcount,
        loginuserid
      );
    }
  ...
  const addOrGetEstimateHistoryData = async () => {
    ...
    if (estimateHistoryData !== null) {
      ...
      if (iscontains === false) {
        setestimateHistoryData([...estimateHistoryData, singleEstimation]);
      ...
      setestimateHistoryData([singleEstimation]);
    }
  ...
  const updateestimateDB = async (data) => {
    ...
    if (isbackendconnect) {
      setisloaded(false);
      let storedataindb = await estimateDetailsDb.saveEstimateDB(
        data,
        loginuserid
      );
      ...
      if (
        storedataindb.status !== 200 ||
        storedataindb.data !== "estimation saved"
      ) {
    ...
    if (type === "New") {
      ...
      if (available.length > 0) {
        ...
        let exitsingleestimatevalue1 = {
          ...
          area: area,
          perqsft: perqsft,
          pvccostpsf: pvccostpsf,
          totalpvccost: totalpvccost,
          upvccostpsf: upvccostpsf,
          isotheritem: isotheritem,
          hideotheritem: hideotheritem,
          totalupvccost: totalupvccost,
          woodcostpsf: woodcostpsf,
          totalwoodcost: totalwoodcost,
          remarks: remarks,
        };
        ...
        available[0] = {
          ...
          title: title,
          orderno: orderno,
          sumtotalpvccost: calculatetotal(
            available[0].sumtotalpvccost,
            totalpvccost,
            "sum",
            2
          ),
          sumtotalupvccost: calculatetotal(
            available[0].sumtotalupvccost,
            totalupvccost,
            "sum",
            2
          ),
          sumtotalwoodcost: calculatetotal(
            available[0].sumtotalwoodcost,
            totalwoodcost,
            "sum",
            2
          ),
          sumtotalsqft: calculatetotal(
            available[0].sumtotalsqft,
            perqsft,
            "sum",
            3
          ),
        ...
        let singleestimate = {
          ...
          title: title,
          orderno: orderno,
          sumtotalpvccost: totalpvccost,
          sumtotalupvccost: totalupvccost,
          sumtotalwoodcost: totalwoodcost,
          discountedcheck: discountedcheck,
          discountedText: discountedText,
          discountedTotalupvccost: discountedTotalupvccost,
          discountedTotalpvccost: discountedTotalpvccost,
      ...
      if (available1.length > 0) {
        ...
        if (othersubtableava.length > 0) {
          available1[0] = {
            ...
            sumtotalpvccost: calculatetotal(
              available1[0].sumtotalpvccost,
              subtableava[0].totalpvccost,
              "diff",
              2
            ),
            sumtotalupvccost: calculatetotal(
              available1[0].sumtotalupvccost,
              subtableava[0].totalupvccost,
              "diff",
              2
            ),
            sumtotalwoodcost: calculatetotal(
              available1[0].sumtotalwoodcost,
              subtableava[0].totalwoodcost,
              "diff",
              2
            ),
            sumtotalsqft: calculatetotal(
              available1[0].sumtotalsqft,
              subtableava[0].perqsft,
              "diff",
              3
            ),
  ...
  const estcontext = {
    ...
    setestimatedate,
    grandtotalupvccost,
    setgrandtotalupvccost,
    grandtotalpvccost,
    setgrandtotalpvccost,
    grandtotalwoodcost,
    setgrandtotalwoodcost,
    title,
    settitle,
    subdesc,
    setsubdesc,
    ...
    remarks,
    setremarks,
    addOrUpdateEstimateItemHandler,
    updateTableView,
    estimatedate1,
    setestimatedate1,
    estimateHistoryData,
    setestimateHistoryData,
    addOrGetEstimateHistoryData,
    dateHandler,
    estimateSingleData,
    setestimateSingleData,
    allEstimateEdit,
    isNewDataSaveType,
    setisNewDataSaveType,
    orderno,
    setorderno,
    setval,
    setboxColors,
    sortorder,
    estimateHistroyUpdateFlag,
    setestimateHistroyUpdateFlag,
    isloaded,
    setisloaded,
    cleartallEstimateotal,
    specialItemHandler,
    discountedcheck,
    setdiscountedcheck,
    discountedText,
    setdiscountedText,
    discountedTotalupvccost,
    setdiscountedTotalupvccost,
    discountedTotalpvccost,
    setdiscountedTotalpvccost,
    discountedTotalwoodcost,
    setdiscountedTotalwoodcost,
    handleExportXlsx,
    deleteEstimate,
  };

  return (
    <estimateState.Provider value={estcontext}>
      {children}
    </estimateState.Provider>
  );
};
