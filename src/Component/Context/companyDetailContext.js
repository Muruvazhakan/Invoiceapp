...
const CompanyDetailContext = ({ children }) => {
    ...
    if (loginuser.length > 0 && loginUserPassword.length > 0) {
      ...
      if (logintype === "login") {
        ...
        if (userExsist.status === 200) {
          ...
          let tier =
            userExsist.data.tier !== null ? userExsist.data.tier : null;
          if (tier != null) {
            settier(tier);
            localstore.addOrGetUserdetail(tier, "tier", "save");
          }
          localstore.addOrGetUserdetail(loginuser, "loginuser", "save");
          localstore.addOrGetUserdetail(
            userExsist.data.userid,
            "userid",
            "save"
          );
  ...
  const getAlldataFromDB = async () => {
    ...
    if (loginuserid !== null && loginuserid !== "") {
      ...
      let getEstimationIdfromDb = await estimateDetailsDb.getEstimationId(
        loginuserid
      );
      // console.log(getEstimationIdfromDb );
      if (
        getEstimationIdfromDb.status === 200 &&
        estimateidcounter < getEstimationIdfromDb.data
      ) {
      ...
      let estimateHistoryData = localstore.addOrGetEstimateHistoryData(
        "",
        "get"
      );

      let getestimatefromdb = await estimateDetailsDb.getEstimateDB(
        loginuserid
      );
      ...
      if (getestimatefromdb.status === 200) {
        if (
          estimateHistoryData === null ||
          estimateHistoryData.length <= getestimatefromdb.data.length
        ) {
          //console.log(getestimatefromdb.data);
          localstore.addOrGetEstimateHistoryData(
            getestimatefromdb.data,
            "save"
          );
      ...
      let companyBankDetailslocal = localstore.addOrGetCompanyBankDetailHandler(
        "",
        "get"
      );
      let companyBankDetailsfromdb =
        await companyDetailsDB.getCompanyBankDetails(loginuserid);
      ...
      if (companyBankDetailsfromdb.status === 200) {
        if (
          !companyBankDetailslocal ||
          (companyBankDetailsfromdb.data &&
            companyBankDetailsfromdb.data.length >
              companyBankDetailslocal.length)
        ) {
          localstore.addOrGetCompanyBankDetailHandler(
            companyBankDetailsfromdb.data,
            "save"
          );
          //console.log("company basic details updated");
      ...
      let companyTermsAndConditionDetailslocal =
        localstore.getCompanyTermsAndConditionHandler();
      let companyTermsAndConditionDetailsfromDB =
        await companyDetailsDB.getCompanyTermsAndConditionDetails(loginuserid);
      ...
      if (companyBasicDetailsfromdb.status === 200) {
        if (
          !companyTermsAndConditionDetailslocal ||
          (companyTermsAndConditionDetailsfromDB.data &&
            companyTermsAndConditionDetailsfromDB.data.length >
              companyTermsAndConditionDetailslocal.length)
        ) {
          localstore.addOrUpdateCompanyTermsAndConditionHandler(
            companyTermsAndConditionDetailsfromDB.data,
            "save"
          );
          setcompanydetails(companyTermsAndConditionDetailsfromDB.data);
    ...
    stockDetail.setisloading(false);
    setisloaded(true);
  };
  ...
  const getAlldataOnLogin = (userid) => {
    ...
    let companyBankdetail = localstore.addOrGetCompanyBankDetailHandler(
      "",
      "get"
    );
  ...
  const saveHandler = async (funcs, item, type) => {
    ...
    if (funcs === "addOrUpdateCompanyTermsAndConditionHandler") {
      ...
      if (isbackendconnect) {
        let saveCompanyTermsAndConditionDetailsdb =
          await companyDetailsDB.saveCompanyTermsAndConditionDetails(
            item,
            loginuserid
          );
        ...
        //console.log(saveCompanyTermsAndConditionDetailsdb);
        if (
          saveCompanyTermsAndConditionDetailsdb.status !== 201 &&
          saveCompanyTermsAndConditionDetailsdb.status !== 200
        ) {
          toast.error(
            saveCompanyTermsAndConditionDetailsdb.data + " in saving DB"
          );
        }
      }
    }
    if (funcs === "addOrGetCompanyBankDetailHandler") {
      ...
      if (isbackendconnect) {
        let saveCompanyBankDetaitlsdb =
          await companyDetailsDB.saveCompanyBankDetails(item, loginuserid);
        //console.log('compabankdet');
        //console.log(saveCompanyBankDetaitlsdb);
        if (
          saveCompanyBankDetaitlsdb.status !== 201 &&
          saveCompanyBankDetaitlsdb.status !== 200
        ) {
    ...
    if (funcs === "addOrUpdateCompanyHandler") {
      ...
      if (isbackendconnect) {
        let companyBasicDetails =
          await companyDetailsDB.saveCompanyBasicDetails(
            item,
            loginuserid,
            estimateidcount
          );
  ...
  const companyBankDetailHandler = async (item, type) => {
    ...
    if (
      companyBankdetailtitle.length === 0 &&
      companyBankdetailvalue.length === 0 &&
      type !== "delete"
    ) {
      toast.error("Both Details are Empty");
      return;
    ...
    if (type === "new") {
      getresul = {
        id: uuidv4(),
        title: companyBankdetailtitle,
        isvisible: companyBankdetailIsVisible,
        value: companyBankdetailvalue,
      };
      //console.log('getresul');
      //console.log(getresul);
      //console.log(companydetails);
      let compabankdet = [...companyBankdetails, getresul];
      //console.log('compabankdet');
      //console.log(compabankdet);
  ...
  useEffect(() => {
    ...
    if (useralreadyloggedin !== null && useralreadyloggedin !== "") {
      ...
      if (tieruserids !== null) settier(tieruserids);
