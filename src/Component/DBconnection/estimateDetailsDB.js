...
export const getEstimationId = async (userid) => {
  ...
  try {
    response = await axios.get(
      `${dbprop.getEstimationIdUrl}/${userid}`,
      config
    );
    // console.log(response);
    return response;
...
export const saveEstimationId = async (estimationcount, userid) => {
  ...
  try {
    response = await axios.post(
      `${dbprop.saveEstimationIdUrl}/${userid}`,
      data,
      config
    );
    // console.log(response);
...
export const deleteEstimateDB = async (estimate, userid) => {
  // console.log(`${dbprop.saveEstimateUrl}/${userid}` + ' dbprop.userLoginUrl');
  const data = {
    estimate,
  };
  ...
  try {
    response = await axios.post(
      `${dbprop.deleteEstimateUrl}/${userid}`,
      data,
      config
    );
    // console.log(response);
